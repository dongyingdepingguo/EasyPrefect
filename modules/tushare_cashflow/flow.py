# -*- coding: utf-8 -*-
"""从 Tushare Pro 获取 cashflow_vip 现金流量表数据。"""
...
def _clean_cashflow_vip_params(
    *,
    ts_code: str = "",
    ann_date: str = "",
    f_ann_date: str = "",
...
@flow(name="Tushare 现金流量表 VIP")
def tushare_cashflow_flow(
    ts_code: str = "",
    ann_date: str = "",
    f_ann_date: str = "",
    start_date: str = "",
    ...
    """获取 Tushare cashflow_vip 现金流量表数据，并按配置写入 ClickHouse。
    ...
    params = _clean_cashflow_vip_params(
        ts_code=ts_code,
        ann_date=ann_date,
        f_ann_date=f_ann_date,
        start_date=start_date,
    ...
    if not isinstance(df, pd.DataFrame):
        raise TypeError(f"cashflow_vip 返回了非预期类型: {type(df)!r}")

    logger.info(
        "已获取 %s 条 Tushare cashflow_vip 数据，参数=%s",
        len(df),
        params,
    )
    write_config = ClickHouseWriteConfig.from_mapping(
        module_runtime("tushare_cashflow", "clickhouse")
    )
    written = write_dataframe(df, write_config)
    if write_config.enabled:
        logger.info(
            "已写入 %s 条 cashflow_vip 数据到 ClickHouse 表 %s，模式=%s",
            written,
            write_config.table,
            write_config.mode,
        )
