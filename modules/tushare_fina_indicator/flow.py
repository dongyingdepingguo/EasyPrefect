# -*- coding: utf-8 -*-
"""从 Tushare Pro 获取 fina_indicator_vip 财务指标数据。"""
...
def _clean_fina_indicator_vip_params(
    *,
    ts_code: str = "",
    ann_date: str = "",
    start_date: str = "",
...
def _query_fina_indicator_vip(
    ...
    *,
    ...
    params = _clean_fina_indicator_vip_params(
        ts_code=ts_code,
        ann_date=ann_date,
        start_date=start_date,
        end_date=end_date,
    ...
    if fields:
        df = pro.fina_indicator_vip(**params, fields=fields)
    else:
        df = pro.fina_indicator_vip(**params)
    if not isinstance(df, pd.DataFrame):
        raise TypeError(
            f"fina_indicator_vip 返回了非预期类型: {type(df)!r}，参数={params}"
        )
    return df.drop_duplicates(ignore_index=True)


@flow(name="Tushare 财务指标 VIP")
def tushare_fina_indicator_flow(
    ts_code: str = "",
    ann_date: str = "",
    start_date: str = "",
    end_date: str = "",
    ...
    """获取 Tushare fina_indicator_vip 财务指标数据，并按配置写入 ClickHouse。
    ...
    write_config = ClickHouseWriteConfig.from_mapping(
        module_runtime("tushare_fina_indicator", "clickhouse")
    )
    fields = _fina_indicator_fields_from_clickhouse(write_config)
    ...
    logger.info(
        "已获取 %s 条 Tushare fina_indicator_vip 数据，参数=%s，字段=%r",
        len(df),
        params,
        list(df.columns),
    )
    ...
    if write_config.enabled:
        logger.info(
            "已写入 %s 条 fina_indicator_vip 数据到 ClickHouse 表 %s，模式=%s",
            written,
            write_config.table,
            write_config.mode,
        )
