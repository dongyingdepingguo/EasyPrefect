# -*- coding: utf-8 -*-
"""从 Tushare Pro 获取 ci_daily 中信行业指数日线行情数据。"""
...
NUMERIC_COLUMNS = tuple(
    column for column in CI_DAILY_COLUMNS if column not in {"ts_code", "trade_date"}
)


def _clean_ci_daily_params(
    *,
    ts_code: str = "",
    trade_date: str = "",
    start_date: str = "",
...
def _create_table_sql(table: str) -> str:
    ...
    columns = [
        f"{quote_identifier('ts_code')} String COMMENT '中信行业指数代码'",
...
def _ensure_ci_daily_table(config: ClickHouseWriteConfig) -> None:
...
def _ci_daily_fields_from_clickhouse(config: ClickHouseWriteConfig) -> str:
    if not config.enabled:
        return ""
...
def _normalize_ci_daily_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
...
def _query_ci_daily_page(
    ...
    *,
    ...
    params: dict[str, Any] = _clean_ci_daily_params(
        ts_code=ts_code,
        trade_date=trade_date,
        start_date=start_date,
        end_date=end_date,
    ...
    if fields:
        df = pro.ci_daily(**params, fields=fields)
    else:
        df = pro.ci_daily(**params)
    if not isinstance(df, pd.DataFrame):
        raise TypeError(f"ci_daily 返回了非预期类型: {type(df)!r}，参数={params}")
    return df


def _query_ci_daily_all(
    pro: Any,
    *,
    ts_code: str,
    trade_date: str,
    ...
    offset = 0
    while True:
        current_df = _query_ci_daily_page(
            pro,
            ts_code=ts_code,
            trade_date=trade_date,
            start_date=start_date,
    ...
    if frames:
        ...
        return _normalize_ci_daily_dataframe(df)
    if empty_template is not None:
        return _normalize_ci_daily_dataframe(empty_template)
    return pd.DataFrame()


@flow(name="Tushare 中信行业指数日线行情")
def tushare_ci_daily_flow(
    ts_code: str = "",
    trade_date: str = "",
    start_date: str = "",
    end_date: str = "",
    ...
    """获取 Tushare ci_daily 中信行业指数日线行情数据，并写入 ClickHouse。

    参数:
        ts_code: 中信行业指数代码，例如 CI005001.CI；为空时不按指数过滤。
    ...
    write_config = ClickHouseWriteConfig.from_mapping(
        module_runtime("tushare_ci_daily", "clickhouse")
    )
    _ensure_ci_daily_table(write_config)
    fields = _ci_daily_fields_from_clickhouse(write_config)
    trade_date = _default_trade_date(trade_date)
    params = _clean_ci_daily_params(
        ts_code=ts_code,
        trade_date=trade_date,
        start_date=start_date,
        end_date=end_date,
    )
    df = _query_ci_daily_all(
        pro,
        ts_code=ts_code,
        trade_date=trade_date,
        start_date=start_date,
    ...
    logger.info(
        "已获取 %s 条 Tushare ci_daily 数据，参数=%s，字段=%r",
        len(df),
        params,
        list(df.columns),
    )
    ...
    if write_config.enabled:
        logger.info(
            "已写入 %s 条 ci_daily 数据到 ClickHouse 表 %s，模式=%s",
            written,
            write_config.table,
            write_config.mode,
        )
