# -*- coding: utf-8 -*-
"""从 Tushare Pro 获取 index_member_all 申万行业成分数据。"""
...
STRING_COLUMNS = tuple(
    column for column in INDEX_MEMBER_ALL_COLUMNS if column not in DATE_COLUMNS
)


def _clean_index_member_all_params(
    *,
    l1_code: str = "",
    l2_code: str = "",
    l3_code: str = "",
...
(
    {quote_identifier('l1_code')} String COMMENT '申万一级行业代码',
    {quote_identifier('l1_name')} String COMMENT '申万一级行业名称',
    {quote_identifier('l2_code')} String COMMENT '申万二级行业代码',
    {quote_identifier('l2_name')} String COMMENT '申万二级行业名称',
    {quote_identifier('l3_code')} String COMMENT '申万三级行业代码',
    {quote_identifier('l3_name')} String COMMENT '申万三级行业名称',
...
def _ensure_index_member_all_table(config: ClickHouseWriteConfig) -> None:
...
def _index_member_all_fields_from_clickhouse(config: ClickHouseWriteConfig) -> str:
    if not config.enabled:
        return ""
...
def _normalize_index_member_all_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
...
def _query_index_member_all_page(
    pro: Any,
    *,
    l1_code: str,
    l2_code: str,
    ...
    params: dict[str, Any] = _clean_index_member_all_params(
        l1_code=l1_code,
        l2_code=l2_code,
        l3_code=l3_code,
        ts_code=ts_code,
    ...
    if fields:
        df = pro.index_member_all(**params, fields=fields)
    else:
        df = pro.index_member_all(**params)
    if not isinstance(df, pd.DataFrame):
        raise TypeError(f"index_member_all 返回了非预期类型: {type(df)!r}，参数={params}")
    return df


def _query_index_member_all_all(
    pro: Any,
    *,
    l1_code: str,
    l2_code: str,
    ...
    offset = 0
    while True:
        current_df = _query_index_member_all_page(
            pro,
            l1_code=l1_code,
            l2_code=l2_code,
            l3_code=l3_code,
    ...
    if frames:
        ...
        return _normalize_index_member_all_dataframe(df)
    if empty_template is not None:
        return _normalize_index_member_all_dataframe(empty_template)
    return pd.DataFrame()


@flow(name="Tushare 申万行业成分")
def tushare_index_member_all_flow(
    l1_code: str = "",
    l2_code: str = "",
    l3_code: str = "",
    ts_code: str = "",
    ...
    """获取 Tushare index_member_all 申万行业成分数据，并写入 ClickHouse。

    参数:
        l1_code: 申万一级行业代码；为空时不按一级行业过滤。
        l2_code: 申万二级行业代码；为空时不按二级行业过滤。
        l3_code: 申万三级行业代码；为空时不按三级行业过滤。
    ...
    write_config = ClickHouseWriteConfig.from_mapping(
        module_runtime("tushare_index_member_all", "clickhouse")
    )
    _ensure_index_member_all_table(write_config)
    fields = _index_member_all_fields_from_clickhouse(write_config)
    params = _clean_index_member_all_params(
        l1_code=l1_code,
        l2_code=l2_code,
        l3_code=l3_code,
        ts_code=ts_code,
    ...
    df = _query_index_member_all_all(
        pro,
        l1_code=l1_code,
        l2_code=l2_code,
        l3_code=l3_code,
    ...
    logger.info(
        "已获取 %s 条 Tushare index_member_all 数据，参数=%s，字段=%r",
        len(df),
        params,
        list(df.columns),
    )
    ...
    if write_config.enabled:
        logger.info(
            "已写入 %s 条 index_member_all 数据到 ClickHouse 表 %s，模式=%s",
            written,
            write_config.table,
            write_config.mode,
        )
