# -*- coding: utf-8 -*-
"""从 Tushare Pro 获取 index_weekly 指数周线行情数据。"""
...
NUMERIC_COLUMNS = tuple(
    column for column in INDEX_WEEKLY_COLUMNS if column not in {"ts_code", "trade_date"}
)


def _clean_index_weekly_params(
    *,
    ts_code: str = "",
    trade_date: str = "",
    start_date: str = "",
...
def _ensure_index_weekly_table(config: ClickHouseWriteConfig) -> None:
...
def _index_weekly_fields_from_clickhouse(config: ClickHouseWriteConfig) -> str:
    if not config.enabled:
        return ""
...
def _query_index_weekly_page(
    ...
    *,
    ...
    params: dict[str, Any] = _clean_index_weekly_params(
        ts_code=ts_code,
        trade_date=trade_date,
        start_date=start_date,
        end_date=end_date,
    ...
    if fields:
        df = pro.index_weekly(**params, fields=fields)
    else:
        df = pro.index_weekly(**params)
    if not isinstance(df, pd.DataFrame):
        raise TypeError(f"index_weekly 返回了非预期类型: {type(df)!r}，参数={params}")
    return df


def _query_index_weekly_all(
    pro: Any,
    *,
    ts_code: str,
    trade_date: str,
    ...
    offset = 0
    while True:
        current_df = _query_index_weekly_page(
            pro,
            ts_code=ts_code,
            trade_date=trade_date,
            start_date=start_date,
...
@flow(name="Tushare 指数周线行情")
def tushare_index_weekly_flow(
    ts_code: str = "",
    trade_date: str = "",
    start_date: str = "",
    end_date: str = "",
    ...
    """获取 Tushare index_weekly 指数周线行情数据，并写入 ClickHouse。
    ...
    write_config = ClickHouseWriteConfig.from_mapping(
        module_runtime("tushare_index_weekly", "clickhouse")
    )
    _ensure_index_weekly_table(write_config)
    fields = _index_weekly_fields_from_clickhouse(write_config)
    trade_date = _default_trade_date(trade_date)
    params = _clean_index_weekly_params(
        ts_code=ts_code,
        trade_date=trade_date,
        start_date=start_date,
        end_date=end_date,
    )
    df = _query_index_weekly_all(
        pro,
        ts_code=ts_code,
        trade_date=trade_date,
        start_date=start_date,
    ...
    logger.info(
        "已获取 %s 条 Tushare index_weekly 数据，参数=%s，字段=%r",
        len(df),
        params,
        list(df.columns),
    )
    ...
    if write_config.enabled:
        logger.info(
            "已写入 %s 条 index_weekly 数据到 ClickHouse 表 %s，模式=%s",
            written,
            write_config.table,
            write_config.mode,
        )
