# -*- coding: utf-8 -*-
"""从 Tushare Pro 获取 cyq_chips 股票每日筹码分布数据。"""
...
(
    ...
    {quote_identifier('trade_date')} Date COMMENT '交易日期',
    {quote_identifier('price')} Float64 COMMENT '成本价格',
    {quote_identifier('percent')} Nullable(Float64) COMMENT '价格占比',
...
ORDER BY (trade_date, ts_code, price)
SETTINGS index_granularity = 8192
COMMENT '股票每日筹码分布'
""".strip()


def _ensure_cyq_chips_table(config: ClickHouseWriteConfig) -> None:
...
def _cyq_chips_fields_from_clickhouse(config: ClickHouseWriteConfig) -> str:
    if not config.enabled:
        return ""
...
def _query_cyq_chips(
    ...
    *,
    ...
    params = _clean_cyq_chips_params(
        ts_code=ts_code,
        trade_date=trade_date,
        start_date=start_date,
        end_date=end_date,
    ...
    if fields:
        df = pro.cyq_chips(**params, fields=fields)
    else:
        df = pro.cyq_chips(**params)
    if not isinstance(df, pd.DataFrame):
        raise TypeError(f"cyq_chips 返回了非预期类型: {type(df)!r}，参数={params}")
    return df.drop_duplicates(ignore_index=True)


@flow(name="Tushare 股票每日筹码分布")
def tushare_cyq_chips_flow(
    ts_code: str = "",
    trade_date: str = "",
    start_date: str = "",
    end_date: str = "",
    ...
    """获取 Tushare cyq_chips 股票每日筹码分布数据，并写入 ClickHouse。
    ...
    write_config = ClickHouseWriteConfig.from_mapping(
        module_runtime("tushare_cyq_chips", "clickhouse")
    )
    _ensure_cyq_chips_table(write_config)
    fields = _cyq_chips_fields_from_clickhouse(write_config)
    ...
    logger.info(
        "已获取 %s 条 Tushare cyq_chips 数据，股票数=%s，参数=%s，字段=%r",
        len(df),
        len(target_ts_codes),
        params,
        list(df.columns),
    ...
    if write_config.enabled:
        logger.info(
            "已写入 %s 条 cyq_chips 数据到 ClickHouse 表 %s，模式=%s",
            written,
            write_config.table,
            write_config.mode,
        )
